...
export default class ExtensionPreviewURL {
  ...
  public static async previewUrlExtensionHandler(options) {
    try {
      ...
      const partner_access_token = getPartnerAccessToken();
      ...
      if (currentEnv != contextEnv) extensionContext.deleteAll();
      ...
      if (!extension.options.apiKey) {
        const selected = await selectExtensionFromList();
      ...
      timer = setInterval(async () => {
        ...
        await extension.checkPorts(ports, () => {
          // get preview URL
  ...
  private getPreviewURL() {
    const baseURL = getPlatformUrls().platform;
    return urljoin(
      baseURL,
  ...
  public getServicePorts(projectConfigs, backendPort:string, frontendPort:string) {
    const ports = [];
    for (const projectConfig of projectConfigs) {
      if (projectConfig.roles.includes('frontend')) ports.push(frontendPort);
      else if (projectConfig.roles.includes('backend')) ports.push(backendPort);
      else if (projectConfig.roles.includes('backend') && projectConfig.roles.includes('frontend')) ports.push(frontendPort);
    }
    return ports;
  }
  ...
  public readAndValidateFDKExtConfigFiles() {
    const projectConfigFiles = findAllFilePathFromCurrentDirWithName(['fdk.ext.config.json', 'fdk.ext.config.yml']);
...
Or you can use fdk tunnel --port <port> command to start tunnel\n`);
      ...
      for (const fdkConfigFilePath of projectConfigFiles) {
