...
import {
  Object, getPartnerAccessToken, selectExtensionFromList, validateEmpty,
...
export default class ExtensionLaunchURL {
  ...
  public static async updateLaunchURL(
    ...
    try {
      const spinner = new Spinner('Updating Launch URL on Partners Panel');
      try {
        spinner.start();
  ...
  public static async getLaunchURLHandler(options: Object) {
    try {
      ...
      const spinner = new Spinner('Fetching Launch URL');
      try {
        spinner.start();
        const extension_data = await ExtensionService.getExtensionDataPartners(
          options.apiKey,
        );
        const launchURL: string = extension_data.base_url;
