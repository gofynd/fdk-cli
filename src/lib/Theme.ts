...
import {
  syncLocales, hasAnyDeltaBetweenLocalAndRemoteLocales, SyncMode, updateLocaleFiles,
} from '../helper/locales';
...
export default class Theme {
  ...
  public static async selectCompanyAndStore() {
    ...
    const config: { accountType?: string; application_id?: string; company_id?: string } = {};
    let company_type;
    let companyList;
    let selectedCompany;
    let applicationList;
    ...
    await inquirer.prompt(companyListQuestions).then(async (answers) => {
      try {
        selectedCompany = companyListOptions[answers.selectedCompany]?.company?.uid
  ...
  public static async createVueTheme(
    ...
    try {
      ...
      const available_sections = await Theme.getAvailableSections();
      const settings_schema = await fs.readJSON(
        path.join(
          process.cwd(),
          'theme',
          'config',
      ...
      const settings_data = await fs.readJSON(
        path.join(
          process.cwd(),
          'theme',
          'config',
  ...
  public static async createReactTheme(
    ...
    try {
      ...
      const available_sections = await Theme.getAvailableReactSectionsForSync(parsed.sections, sectionChunkingEnabled);
      const settings_schema = await fs.readJSON(
        path.join(
          process.cwd(),
          'theme',
  ...
  public static initTheme = async (options) => {
    ...
    try {
      ...
      const list = _.get(themeData, 'config.list', []);
      const current = _.get(themeData, 'config.current', 'default');
      const preset = _.get(themeData, 'config.preset', {});
      try {
      ...
      if (!fs.existsSync(path.join(process.cwd(), DEV_VUE_THEME_ENTRY_FILE)) && (themeData.theme_type === THEME_TYPE.vue2)) {
        ...
        const restructureSpinner = new Spinner(
  ...
  private static syncReactTheme = async (
    ...
    try {
      ...
      const srcCdnUrl = await Theme.uploadThemeSrcZip();
      ...
      const pArr = await Theme.uploadReactThemeBundle({ buildPath });
      const [cssUrls, umdJsUrls] = await Promise.all(pArr);
  ...
  private static syncVueTheme = async (
    ...
    try {
      ...
      const available_sections = await Theme.getAvailableSectionsForSync();
      await Theme.validateAvailableSections(available_sections);
      await Theme.validateSectionWithDB(
        theme.available_sections,
        available_sections,
      ...
      Logger.debug('Uploading bundle files');
      const pArr = await Theme.uploadThemeBundle({ assetHash });
      const [cssUrls, commonJsUrl, umdJsUrls] = await Promise.all(pArr);
  ...
  private static commonSetup = async (options) => {
    ...
    const serverPort = typeof options.port === 'string'
    ...
    const { data: appInfo } = await ConfigurationService.getApplicationDetails();
    const domain = Array.isArray(appInfo.domains)
  ...
  public static serveVueTheme = async (options) => {
    try {
      ...
      const watcher = chokidar.watch(path.resolve(process.cwd(), 'theme'), {
        persistent: true,
      });
  ...
  public static serveReactTheme = async (options) => {
    try {
      ...
      const isHMREnabled = typeof options.hmr === 'boolean'
        ? options.hmr
        : options.hmr == 'true';
  ...
  public static pullTheme = async () => {
    const spinner = new Spinner('Pulling theme data');
    try {
      spinner.start();
      ...
      const list = _.get(theme, 'config.list', []);
      const current = _.get(theme, 'config.current', 'default');
      const preset = _.get(theme, 'config.preset', {});
      await Theme.writeSettingJson(Theme.getSettingsDataPath(), {
        list,
        current,
        preset,
  ...
  private static extractSettingsFromFile(path) {
    try {
      const $ = cheerio.load(readFile(path));
  ...
  private static sanitizeThemeName = (name) => name.replace(/ /g, '_');

  private static unSanitizeThemeName = (name) => name.replace(/_/g, ' ');
  ...
  private static uploadThemeBundle = async ({ assetHash }) => {
    try {
      ...
      const cssAssests = glob.sync(
        path.join(process.cwd(), Theme.BUILD_FOLDER, '**.css'),
      );
      const cssPromisesArr = cssAssests.map((asset) => UploadService.uploadFile(
        asset,
        'application-theme-assets',
      ));
  ...
  private static setThemeData = async (
    ...
    // thumbnailImages,
    try {
      const themeContent: any = readFile(
        path.join(process.cwd(), 'config.json'),
      );
      ...
      const globalConfigSchema = await fs.readJSON(
        path.join(
          process.cwd(),
          'theme',
          'config',
      ...
      const globalConfigData = await fs.readJSON(
        path.join(
          process.cwd(),
          'theme',
          'config',
  ...
  private static updateAvailablePages = async ({ assetHash }) => {
    ...
    try {
      ...
      await asyncForEach(systemPagesLocally, async (fileName) => {
        const pageName = fileName.replace('.vue', '');
        // SYSTEM Pages
      ...
      await Promise.all(
        pagesToDelete.map((page) => ThemeService.deleteAvailablePage(page.value)),
      );
  ...
  private static updateAvailablePagesForReact = async (customTemplates) => {
    ...
    try {
      ...
      await asyncForEach(systemPages, async (fileName) => {
        ...
        systemPage.props = Theme.extractSettingsFromReactFile(
          path.join(process.cwd(), 'theme', 'pages', fileName),
        ).props
                    || []
                    || [];
        systemPage.sections_meta = Theme.extractSectionsFromReactFile(
          path.join(process.cwd(), 'theme', 'pages', fileName),
        )
                    || []
                    || [];
      ...
      const allLocalSystemPageNames = systemPages.map((name) => name.replace('.jsx', ''));
  ...
  public static generateAvailablePagesReact = async (
    ...
    try {
      ...
      await asyncForEach(systemPages, async (fileName) => {
        ...
        pageData.props = Theme.extractSettingsFromReactFile(
          path.join(process.cwd(), 'theme', 'pages', fileName),
        ).props
                    || []
                    || [];
        pageData.sections_meta = Theme.extractSectionsFromReactFile(
          path.join(process.cwd(), 'theme', 'pages', fileName),
        )
                    || []
                    || [];
  ...
  public static generateThemeZip = async () => {
    ...
    try {
      ...
      const packageContent: any = readFile(filepath);
      const content = JSON.parse(packageContent) || {};
      process.on('SIGINT', () => {
