...
export default class ExtensionSection {
      ...
      'appliedTheme',
          ...
          try {
            switch (key) {
              ...
              case 'extensionId':
                let extensionId;
                try {
                  const extensionsList = await extensionService.getExtensionList(1, 500);
              ...
              case 'appliedTheme':
                ...
                try {
                  ...
                  const { data: appConfig } = await configurationService.getApplicationDetails(
                    configObj,
                  );
                  ...
                  for (const lkey in themeDetails) {
        ...
        try {
          ...
          const missingKeys = requiredOptions.filter(
            (key) => !Object.prototype.hasOwnProperty.call(mergedConfig, key),
          );

          const userInput = {};
          for (const val in missingKeys) {
  ...
  static createSectionsDirectoryIfNotExists(framework: string): void {
    const directories = framework === 'react'
    ...
    if (isReact) {
      ...
      try {
        const sectionsFiles = fs
          ...
          .filter((o) => o != 'index.js');
        sections = sectionsFiles.map((f) => ExtensionSection.extractSettingsFromFile(
          path.join(
            currentRoot,
  ...
  static async buildExtensionCode({
    ...
    const spinner = new Spinner('Building Extension Code');
    try {
      spinner.start();
  ...
  static async uploadSectionFiles(sectionName: string) {
    ...
    const uploadURLs = {};
    const promises = files.map(([fileExtension, fileName]) => uploadService
      .uploadFile(
      ...
      .then((response) => {
        const { url } = response.complete.cdn;
    ...
    if (bindingInterface === 'Web Theme') {
      if (framework === 'react' || framework === 'vue2') {
        ...
        try {
          ...
          const { _id: extensionSectionId } = await extensionService.getExtensionBindings(
            extensionId,
            organisationId,
            bundleName,
