...
export const URLS = {
  ...
  GET_ALL_THEME: (company_id: number, application_id: string) => urlJoin(
    THEME_URL(),
    `organization/${getOrganizationId()}/company/${company_id}/application/${application_id}/themes`,
  ),

  GET_DEFAULT_THEME: (company_id: number, application_id: string) => urlJoin(
    THEME_URL(),
    `organization/${getOrganizationId()}/company/${company_id}/application/${application_id}/default_theme`,
  ),

  GET_APPLIED_THEME: (company_id: number, application_id: string) => urlJoin(
    THEME_URL(),
  ...
  SETUP_COMPANY: (company_id: number) => urlJoin(
    MIXMASTER_URL('partner'),
    `organization/${getOrganizationId()}/company/${company_id}/setup`,
  ),

  // Extension
  GET_EXTENSION_LIST: (page_no: number, page_size: number): string => urlJoin(
    MIXMASTER_URL('partner'),
    `/organization/${getOrganizationId()}/extension/?page_size=${page_size}&page_no=${page_no}`,
  ),

  REGISTER_EXTENSION_PARTNER: (): string => urlJoin(
    MIXMASTER_URL('partner'),
    `organization/${getOrganizationId()}/extension`,
  ),

  GET_EXTENSION_DETAILS_PARTNERS: (extension_api_key: string): string => urlJoin(
    MIXMASTER_URL('partner'),
    `organization/${getOrganizationId()}/extension/${extension_api_key}`,
  ),

  UPDATE_EXTENSION_DETAILS: (extension_api_key: string): string => urlJoin(
    MIXMASTER_URL('panel'),
    `/extensions/${extension_api_key}`,
  ),

  UPDATE_EXTENSION_DETAILS_PARTNERS: (extension_api_key: string): string => urlJoin(
    MIXMASTER_URL('partner'),
    `organization/${getOrganizationId()}/extension/${extension_api_key}`,
  ),

  // Extension Section
  PUBLISH_SECTIONS: (extension_id: string, organization_id) => urlJoin(
    THEME_URL(),
    `organization/${organization_id}/extension-section/${extension_id}/publish`,
  ),
  DRAFT_SECTIONS: (extension_id: string, organization_id) => urlJoin(
    THEME_URL(),
    `organization/${organization_id}/extension-section/${extension_id}/draft`,
  ),
  PREVIEW_SECTIONS: (extension_id: string, organization_id) => urlJoin(
    THEME_URL(),
    `organization/${organization_id}/extension-section/${extension_id}/preview`,
  ),
  DELETE_SECTIONS: (extension_id: string, organization_id) => urlJoin(
    THEME_URL(),
    `organization/${organization_id}/extension-section/${extension_id}/preview`,
  ),
  GET_EXTENSION_SECTIONS: (extension_id: string, organization_id: string, binding_name: string, accountType: string, framework: string) => urlJoin(
    THEME_URL(),
    `organization/${organization_id}/extension-section/${extension_id}/${binding_name}?accountType=${accountType}&type=${framework}`,
  ),
  // Organization
  GET_ORGANIZATION_DETAILS: () : string => urlJoin(
    MIXMASTER_URL('partner'),
    `organization/${getOrganizationId()}`,
  ),

  // Preview URL
  GET_DEVELOPMENT_ACCOUNTS: (page_no: number, page_size: number): string => urlJoin(
    MIXMASTER_URL('partner'),
    `/organization/${getOrganizationId()}/accounts?page_size=${page_size}&page_no=${page_no}`,
  ),
  GET_LIVE_ACCOUNTS: (page_no: number, page_size: number): string => urlJoin(
    MIXMASTER_URL('partner'),
