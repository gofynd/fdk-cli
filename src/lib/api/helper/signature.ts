...
class RequestSigner {
        ...
        if (request.signQuery) {
            ...
            if (!request.doNotModifyHeaders) {
                if (headers['x-fp-date']) {
                    this.datetime =
                        headers['x-fp-date'] || headers['x-fp-date'];
                } else {
    ...
    canonicalHeaders() {
        ...
        return Object.keys(headers)
            .filter(function (key) {
                let notInIgnoreHeader =
                    HEADERS_TO_IGNORE[key.toLowerCase()] == null;
                if (notInIgnoreHeader) {
                    let foundMatch = false;
                    for (let t in HEADERS_TO_INCLUDE) {
                        foundMatch =
                            foundMatch ||
                            new RegExp(HEADERS_TO_INCLUDE[t], 'ig').test(key);
                    }
                    return foundMatch;
                } else {
                    return false;
