...
export const decodeBase64 = (encodedString) => Buffer.from(encodedString, 'base64').toString('utf8');
...
export const createContext = async (context) => {
  try {
    ...
    const contextsData = await fs.readJSON(CONTEXT_PATH());
...
export const isAThemeDirectory = () => fs.existsSync(FDK_PATH());
...
export const pageNameModifier = (page) => {
  const pageArr = (page[0] === ':' ? page.substring(1) : page).split('-');
  let res = '';
  pageArr.forEach((p) => {
...
export const requireFile = (path) => require(path);

export const evaluateModule = (code) => {
  const script = new vm.Script(NativeModule.wrap(code), {
    displayErrors: true,
  } as vm.ScriptOptions);
  const compiledWrapper = script.runInNewContext();
...
export const installJavaPackages = async (
  ...
  const exec = execa('mvn', ['package', '-DskipTests'], { cwd: targetDir });
...
export const installNpmPackages = async (targetDir: string = process.cwd()) => new Promise(async (resolve, reject) => {
  const exec = execa('npm', ['i'], { cwd: targetDir });
  exec.stdout.on('data', (data) => {
    Debug(data);
  });
  exec.stderr.on('data', (data) => {
...
export const isNetworkErrorCode = (code) => ['ECONNABORTED', 'EPIPE', 'ENOTFOUND', 'ETIMEDOUT', 'ECONNRESET'].includes(
  code,
);
