...
export default (ctx, extendedWebpackConfig): Configuration[] => {
  ...
  const assetNormalizedBasePath = assetBasePath[assetBasePath.length - 1] === '/'
    ? assetBasePath
    : `${assetBasePath}/`;
  const imageCDNNormalizedBasePath = imageCdnUrl[imageCdnUrl.length - 1] === '/'
    ? imageCdnUrl
    : `${imageCdnUrl}/`;
  ...
  if (mergedBaseConfig.entry.hasOwnProperty('themeBundle')) {
    const entryPoints = [...(mergedBaseConfig.entry as any).themeBundle];
    ...
    (mergedBaseConfig.entry as any).themeBundle = entryPoints;
  }
