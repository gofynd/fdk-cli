...
export const getPartnerAccessToken = (): string => configStore.get(CONFIG_KEYS.PARTNER_ACCESS_TOKEN);
...
export const getCompanyId = async (promptMessage = undefined) => {
  const developmentCompanyData = await ExtensionService.getDevelopmentAccounts(
    1,
    9999,
  );
...
export const replaceContent = (
  ...
  replaceStr: string,
): string => content.replace(new RegExp(`${searchPattern}`, 'g'), replaceStr);
