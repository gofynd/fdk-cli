...
describe('React Theme Commands', () => {
  beforeAll(async () => {
    ...
    mock.onGet(initReactThemeData.src.link).reply(() => [200, fs.createReadStream(filePath)]);
    ...
    const zipfilePath = path.join(__dirname, 'fixtures', 'Locale_Turbo.zip');
    mock.onGet(pullReactThemeData.src.link).reply(() => [200, fs.createReadStream(zipfilePath)]);
    mockInstance.onGet(pullReactThemeData.src.link).reply(() => [200, fs.createReadStream(zipfilePath)]);
    mock.onDelete(
      `${URLS.THEME_BY_ID(
        appConfig.application_id,
        appConfig.company_id,
