...
jest.mock('configstore', () => {
    ...
    return class MockConfigstore {
        store = new Store('test-cli', undefined, {
            configPath: path.join(__dirname, 'previewUrl-test-cli.json'),
        });
        all = this.store.all;
        get(key: string) {
...
describe('Extension preview-url command', () => {
    ...
    beforeEach(async () => {
        ...
        mockAxios.onGet(`${URLS.GET_ORGANIZATION_DATA(TOKEN)}`).reply(200, { id: ORGANIZATION_ID });
    ...
    afterEach(async () => {
        ...
        const path = jest.requireActual<typeof import('path')>('path');
        rimraf.sync(path.join(__dirname, 'previewUrl-test-cli.json'));
    });
    ...
    it('should throw port error', async () => {
        ...
        try {
            ...
            await program.parseAsync(['ts-node', './src/fdk.ts', 'extension', 'preview-url']);
        } catch (error) {
    ...
    it('should prompt for ngrok url and return preview url', async () => {
        ...
        jest.spyOn(inquirer, 'prompt').mockResolvedValueOnce({ ngrok_authtoken: 'auth_token' });
