...
import { readFile, createDirectory } from '../helper/file.utils';
...
async function createThemeFromZip() {
  const zipPath = path.join(__dirname, 'fixtures', 'rolex.zip');
  const destination = path.join(__dirname, '..', '..', 'test-theme');
  await extractArchive({ zipPath, destFolderPath: destination });
  if (!fs.existsSync(path.join(process.cwd(), '.fdk'))) await fs.mkdir(path.join(process.cwd(), '.fdk'));
...
describe('Theme Commands', () => {
  beforeAll(async () => {
    ...
    const filePath = path.join(__dirname, 'fixtures', 'archive.zip');
    //
    mockInstance
      .onGet(
    ...
    mock.onGet(initThemeData.src.link).reply(() => [200, fs.createReadStream(filePath)]);
    ...
    const zipfilePath = path.join(__dirname, 'fixtures', 'pull-archive.zip');
    mock.onGet(pullThemeData.src.link).reply(() => [200, fs.createReadStream(zipfilePath)]);
    mockInstance.onGet(pullThemeData.src.link).reply(() => [200, fs.createReadStream(zipfilePath)]);
    mock.onDelete(
      `${URLS.THEME_BY_ID(
        appConfig.application_id,
        appConfig.company_id,
  ...
  it('should successfully generate .zip file of theme', async () => {
    ...
    const fileName = `${content.name}_${content.version}.zip`;
