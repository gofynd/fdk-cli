...
| Command                   | Description              |
| ------------------------- | ------------------------ |
| [status](#service-status) | Check all service status |
...
This command is used to check whether all services are working or not.

#### **Syntax**

```sh
fdk status check [options]
```
...
```sh
fdk status check --name [service_name]
```
